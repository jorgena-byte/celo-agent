...
// SAFE EXECUTION
// ============================================================
async function executeSafeSwap(fromSymbol, toSymbol, amount) {
...
async function checkFxStrategy() {
  ...
  for (const pair of pairs) {
    ...
    if (absSpread >= CONFIG.FX_MIN_SPREAD_PCT) {
      ...
      try {
        await executeSafeSwap(sellSym, buySym, CONFIG.FX_TRADE_SIZE_USD);
...
async function sendDailyReport() {
  ...
  if (yTrades.length === 0) {
    ...
    report += `💱 Total FX yield: $${totalFxYield.toFixed(4)}\n\n`;
    report += '<b>Trade Log:</b>\n';
    yTrades.forEach(t => {
      const time = new Date(t.timestamp).toTimeString().slice(0, 5);
      report += `• ${time} — ${t.amount} ${t.fromSymbol} → ${t.toSymbol}\n`;
    });
  }
...
function scheduleDaily8am(fn) {
  ...
  setTimeout(() => { fn(); setInterval(fn, 24 * 60 * 60 * 1000); }, msUntil8am());
}
